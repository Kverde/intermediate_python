...
| set.disdisjoint(other)                   | Возвращает **True** если множества не имеют общих элементов. Множества являются непересекающимися (disjoint) тогда и только тогда когда их пересечение равно пустому множеству. |
| set.issubset(other),  set <= other       | Проверяет что каждый элемент из множества есть в другом множестве. |
| set < other                              | Проверяет что множество является строгим подмножеством другого множества (`set <= other` и `set != other`). |
| set.ssuperset(other), set >= other       | Проверяет что каждый элемент в другом множестве есть в текущем множестве. |
| set > other                              | Проверяет что другое множество является строгим подмножеством текущего множества (`set >= other` и `set != other`). |
| set.union(other, …), set \| other …      | Возвращает новое множество — объединение текущего и указанных множеств. |
